...
None <- read_csv('data/None_words.csv')
Sex <- read_csv('data/Sex_words.csv')
Mental_Health <- read_csv('data/Mental_words.csv')
Money_Financial <- read_csv('data/Money_words.csv')
Medical <- read_csv('data/Medical_words.csv')
Drugs <- read_csv('data/Drugs_words.csv')
Race_ProtectedGroups <- read_csv('data/Race_words.csv')
Excretions <- read_csv('data/Excretions_words.csv')
Death <- read_csv('data/Death_words.csv')
...
None_total_word_count <- length(None)
Sex_total_word_count <- length(Sex)
Mental_total_word_count <- length(Mental_Health)
Money_total_word_count <- length(Money_Financial)
Medical_total_word_count <- length(Medical)
Drugs_total_word_count <- length(Drugs)
Race_total_word_count <- length(Race_ProtectedGroups)
Excretions_total_word_count <- length(Excretions)
Academics_total_word_count <- length(Academics)
Death_total_word_count <- length(Death)

distinct_word_count <- n_distinct(all_tokens)
None_distinct_word_count <- n_distinct(None)
Sex_distinct_word_count <- n_distinct(Sex)
Mental_distinct_word_count <- n_distinct(Mental_Health)
Money_distinct_word_count <- n_distinct(Money_Financial)
Medical_distinct_word_count <- n_distinct(Medical)
Drugs_distinct_word_count <- n_distinct(Drugs)
Race_distinct_word_count <- n_distinct(Race_ProtectedGroups)
Excretions_distinct_word_count <- n_distinct(Excretions)
Academics_distinct_word_count <- n_distinct(Academics)
Death_distinct_word_count <- n_distinct(Death)
...
None <- tibble(None_words = names(table(None)), count = table(None)) %>% 
  mutate(perc_total = round(count / None_total_word_count, 4)
         ,perc_distinct = round(count / None_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Sex <- tibble(Sex_words = names(table(Sex)), count = table(Sex)) %>% 
  mutate(perc_total = round(count / Sex_total_word_count, 4)
         ,perc_distinct = round(count / Sex_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Mental_Health <- tibble(Mental_words = names(table(Mental_Health)), count = table(Mental_Health)) %>% 
  mutate(perc_total = round(count / Mental_total_word_count, 4)
         ,perc_distinct = round(count / Mental_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Money_Financial <- tibble(Money_words = names(table(Money_Financial)), count = table(Money_Financial)) %>% 
  mutate(perc_total = round(count / Money_total_word_count, 4)
         ,perc_distinct = round(count / Money_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Medical <- tibble(Medical_words = names(table(Medical)), count = table(Medical)) %>% 
  mutate(perc_total = round(count / Medical_total_word_count, 4)
         ,perc_distinct = round(count / Medical_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Drugs <- tibble(Drugs_words = names(table(Drugs)), count = table(Drugs)) %>% 
  mutate(perc_total = round(count / Drugs_total_word_count, 4)
         ,perc_distinct = round(count / Drugs_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Race_ProtectedGroups <- tibble(Race_words = names(table(Race_ProtectedGroups)), count = table(Race_ProtectedGroups)) %>% 
  mutate(perc_total = round(count / Race_total_word_count, 4)
         ,perc_distinct = round(count / Race_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Excretions <- tibble(Excretions_words = names(table(Excretions)), count = table(Excretions)) %>% 
  mutate(perc_total = round(count / Excretions_total_word_count, 4)
         ,perc_distinct = round(count / Excretions_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Academics <- tibble(Academics_words = names(table(Academics)), count = table(Academics)) %>% 
  mutate(perc_total = round(count / Academics_total_word_count, 4)
         ,perc_distinct = round(count / Academics_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))

Death <- tibble(Death_words = names(table(Death)), count = table(Death)) %>% 
  mutate(perc_total = round(count / Death_total_word_count, 4)
         ,perc_distinct = round(count / Death_distinct_word_count, 4)) %>% 
  filter(!words %in% stopwords('english')) %>%
  arrange(desc(count))
...
top_1000_words <- bind_cols(None[1:1000,1]
                            , Sex[1:1000,1]
                            , Mental_Health[1:1000,1]
                            , Money_Financial[1:1000,1]
                            , Medical[1:1000,1]
                            , Drugs[1:1000,1]
                            , Race_ProtectedGroups[1:1000,1]
                            , Excretions[1:1000,1]
                            , Academics[1:1000,1]
                            , Death[1:1000,1])
